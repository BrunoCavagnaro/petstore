<?php include_once 'includes/templates/header.php'; ?>

  <!-- Hero -->
  <header class="hero text-white text-center">
    <div class="overlay"></div>
    <div class="container">
      <div class="row">
        <div class="col-xl-9 mx-auto">
          <h1 class="my-5">Nosotros te ayudamos a cuidar y amar a tu mascota</h1>
        </div>
        <div class="col-md-10 col-lg-8 col-xl-7 mx-auto">
          <a href="products.php" class="mt-2 btn btn btn-lg btn-success">Ver todos los productos</a>
        </div>
      </div>
    </div>
  </header>

  <!-- Servicios -->
  <section class="services-icons bg-light text-center">
    <div class="container">
      <div class="row">
        <div class="col-lg-4">
          <div class="services-icons-item mx-auto mb-5 mb-lg-0 mb-lg-3">
            <div class="services-icons-icon d-flex">
            <i class="fas fa-bath m-auto text-primary"></i>
            </div>
            <h3>Limpieza</h3>
            <p class="lead mb-0">This theme will look great on any device, no matter the size!</p>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="services-icons-item mx-auto mb-5 mb-lg-0 mb-lg-3">
            <div class="services-icons-icon d-flex">
              <i class="fas fa-briefcase-medical m-auto text-primary"></i>
            </div>
            <h3>Salud</h3>
            <p class="lead mb-0">Featuring the latest build of the new Bootstrap 4 framework!</p>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="services-icons-item mx-auto mb-0 mb-lg-3">
            <div class="services-icons-icon d-flex">
              <i class="fas fa-cookie-bite m-auto text-primary"></i>
            </div>
            <h3>Nutrición</h3>
            <p class="lead mb-0">Ready to use with your own content, or customize the source files!</p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- Casos de Servicios -->
  <section class="showcase">
    <div class="container-fluid p-0">
      <div class="row no-gutters">

        <div class="col-lg-6 order-lg-2 text-white showcase-img" style="background-image: url('img/bg-showcase-1.jpg');"></div>
        <div class="col-lg-6 order-lg-1 my-auto showcase-text">
          <h2>Los mejores productos de limpieza</h2>
          <p class="lead mb-0">When you use a theme created by Start Bootstrap, you know that the theme will look great on any device, whether it's a phone, tablet, or desktop the page will behave responsively!</p>
        </div>
      </div>
      <div class="row no-gutters">
        <div class="col-lg-6 text-white showcase-img" style="background-image: url('img/bg-showcase-2.jpg');"></div>
        <div class="col-lg-6 my-auto showcase-text">
          <h2>Su salud es nuestro compromiso</h2>
          <p class="lead mb-0">Newly improved, and full of great utility classes, Bootstrap 4 is leading the way in mobile responsive web development! All of the themes on Start Bootstrap are now using Bootstrap 4!</p>
        </div>
      </div>
      <div class="row no-gutters">
        <div class="col-lg-6 order-lg-2 text-white showcase-img" style="background-image: url('img/bg-showcase-3.jpg');"></div>
        <div class="col-lg-6 order-lg-1 my-auto showcase-text">
          <h2>Alimentados &amp; Fuertes</h2>
          <p class="lead mb-0">Landing Page is just HTML and CSS with a splash of SCSS for users who demand some deeper customization options. Out of the box, just add your content and images, and your new landing page will be ready to go!</p>
        </div>
      </div>
    </div>
  </section>

<?php include_once 'includes/templates/footer.php'; ?>